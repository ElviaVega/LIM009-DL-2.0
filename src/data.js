...
const edad = año - data.yearOfBirth
...
/*array de objetos con unicamente las propiedades requeridas*/
//const datos = (data) => {
   let arrayHarry1=[];
      dataHarry.forEach((item)=>{
      let person ={};
 ...
 const orderEdad = (array) =>{
  ...
  console.log(orderEdad(arrayHarry1));

  /* ordenamiento especificando porque se quiere ordenar*/
/*
  ...
  console.log(arrayOrder(arrayHarry1, edad));

 */
//FUNCION DE FILTRADO


console.log(Object.values(arrayHarry1[0]));
const filtrado = (array, condicion) => {
  let arrayfiltrado = [];
  for (let i=0; i<array.length; i++){
    arrayValues= Object.values(array[i])
    for (let j=0; j<arrayValues.length; j++){
      if(condicion=== arrayValues[j]){
  ...
  console.log(filtrado(dataHarry, female));
